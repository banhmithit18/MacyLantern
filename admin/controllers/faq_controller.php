...
if ($function == "add_faq") {
    ...
    try {
        $db->Create($faq);
        $log = new log();
        $log->log_name = "Add faq";
        $log->log_detail = "Add faq question: " . $faq_question;
...
if ($function == "update_faq") {
    ...
    if ($db->Update($faq, $faq_id)) {
        $log = new log();
        $log->log_name = "Update faq";
        $log->log_detail = "Update faq with id: " . $faq_id;
        $log->user_id = $_SESSION['user_id'];
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $date = date('Y-m-d H:i:s');
        $log->log_time = $date;
...
if ($function == "delete_faq") {
    ...
    if ($db->Delete($faq)) {
        $log = new Log();
        $log->log_name = "Delete faq";
        $log->log_detail = "Delete faq with id: " . $faq_id;
        $log->user_id = $_SESSION['user_id'];
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $date = date('Y-m-d H:i:s');
        $log->log_time = $date;
