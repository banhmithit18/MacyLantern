...
if($function == "get_image"){
    $db = new DBConnection();
    $sql = "SELECT image.image_id, image.image_path, 'Active' AS status, 'Product' AS type FROM image JOIN product_image ON image.image_id = product_image.image_id
    UNION 
    ...
    WHERE image_id NOT IN( SELECT image.image_id FROM image JOIN blog ON image.image_id = blog.image_id UNION SELECT image.image_id FROM image JOIN product_image ON image.image_id = product_image.image_id UNION SELECT image.image_id FROM image JOIN banner ON image.image_id = banner.image_id )
    ORDER BY image_id";
...
if($function == 'delete_image')
{
    ...
    if($result = $db->Delete($image)){
        ...
        $log = new Log();
        $log->log_detail = "Delete image with id = $image_id";
        $log->log_time = date("Y-m-d H:i:s");
        $log->user_id = $_SESSION['user_id'];
        $log->log_name = "Delete image";
        $db->Create($log);
