...
if ($function == "add_category") {
    ...
    try {
        $db->Create($category);
        $log = new log();
        $log->log_name = "Add category";
        $log->log_detail = "Add category question: " . $category_name;
...
if ($function == "update_category") {
    ...
    if ($db->Update($category, $category_id)) {
        $log = new log();
        $log->log_name = "Update category";
        $log->log_detail = "Update category with id: " . $category_id;
        $log->user_id = $_SESSION['user_id'];
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $date = date('Y-m-d H:i:s');
        $log->log_time = $date;
...
if ($function == "delete_category") {
    ...
    if ($db->Delete($category)) {
        $log = new Log();
        $log->log_name = "Delete category";
        $log->log_detail = "Delete category with id: " . $category_id;
        $log->user_id = $_SESSION['user_id'];
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $date = date('Y-m-d H:i:s');
        $log->log_time = $date;
